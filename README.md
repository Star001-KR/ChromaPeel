...
- **다색 chroma key** — CLI `--target-color` 반복 지정 또는 GUI "+ 색상 추가" 버튼으로 한 이미지에 섞여 있는 여러 배경색(그라데이션 마젠타, 이중 배경 등)을 한 번에 제거. 자동 감지도 다색을 지원하며 feather 영역의 decontaminate 는 픽셀별 가장 가까운 타겟 색 기준으로 적용
...
모든 푸시·PR은 GitHub Actions에서 Ubuntu / Windows / macOS × Python 3.10 / 3.12 / 3.13 매트릭스로 자동 검증됩니다.
